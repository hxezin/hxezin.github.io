---
title: "[Nomad Coders] Typescript로 블록체인 만들기 #3"
...
- `call signatures`: 함수 위에 마우스를 올렸을 때 보이는, 함수 인자의 타입과 반환 값의 타입을 지정하는 문법이다.
...
- `오버로딩`: 하나의 함수가 **서로 다른 여러개의 call signature를 가질 때** 발생한다.
...
- `다형성(Polymorphism)`: 파라미터와 반환값의 타입에 따라 여러 형태를 가진다.
  ```tsx
